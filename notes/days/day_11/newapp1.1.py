...
def get_contact_from_user():
    ''' returns an Address class with data entered from user '''
...
def search_by_name():
    while True:
        ...
        if len(name) != 2:
            ...
            Contact().name.get_name(name)
        return name

def delete_by():
    while True:

        delete_act = input('Would you like to delete by -Name- or by -Address-?').lower().strip(' ')
        if delete_act == 'name':
            ADDRESS_BOOK.delete_contact(get_name_from_user())
            break
        elif delete_act == 'address':
            ADDRESS_BOOK.delete_contact_by_address(get_address_from_user())
            break
        else:
            print(f'"{delete_act}"" is invalid')
            continue
        #return delete_act # WHY YOU RETURNING THIS????? NAME OR ADDRESS? YOU DUMB BOY 
...
while True:
    ...
    if ACTION == 'add':
        ...
        if ADDRESS_BOOK.contacts == []:#<--------------look into using this in other areas.
    ...
    elif ACTION == 'delete':# build here then move build to a function
        delete_by()
